...
# # 3. 重定向所有DNS查询到dnstc
# #iptables -t nat -A REDSOCKS_DNS -p udp --dport 53 -j REDIRECT --to-ports 53
# #iptables -t nat -A REDSOCKS_DNS -p tcp --dport 53 -j REDIRECT --to-ports 53

# #iptables -t nat -A REDSOCKS_DNS -p tcp -j REDIRECT --to-ports 9999
...
# DNS重定向（强制走TCP）
#iptables -t nat -A OUTPUT -p udp --dport 53 -j REDIRECT --to-ports 8888
